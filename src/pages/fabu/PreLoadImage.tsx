...
// 懒加载图片组件（增强版）
const LazyImage = ({ src, alt, width = 80, height = 80, onVisible }) => {
    ...
    return (
        <div ref={imgRef} style={{ width, height, backgroundColor: '#f5f5f5', position: 'relative' }}>
            {inView && (
                <Image
                    src={src}
                    alt={alt}
...
// 图片列渲染（增强版）
const ImageColumn = ({ images, id, onImagesVisible }) => {
    ...
    return (
        <div>
            ...
            <div style={{ marginBottom: '16px', fontSize: '12px', color: '#666' }}>
                已预加载图片: {preloadManager.preloadedImages.size} |
                可视区域图片: {visibleRowImages.size}
            </div>
